<template>
 <AdminNavigation>
   ...
   <div class="container-xxl" style="background-color: var(--background-color);" v-if="owner">
     ...
     <div class="row mt-5 pt-3" v-if="currentProduct">
       <div class="col-12">
         <div class="d-flex align-items-center justify-content-between" >
           ...
           <div class="dropdown">
             ...
             <ul class="dropdown-menu " aria-labelledby="dropdownMenuButton1">
               ...
               <router-link class="dropdown-item text-dark" :to="{name:'Edit',params:{shop_id:owner.id, product_id:currentProduct.id}}">
     ...
     <div class="row">
       <div class="col-12 col-lg-4">
         <div id="carouselExampleControls" class="carousel slide" style="border-radius: var(--border-radius);" data-bs-ride="carousel" v-if="currentProduct">
           <div id="carouselExampleIndicators" class="carousel slide mx-auto" data-bs-ride="carousel">
             ...
             <div class="carousel-inner bg-dark" v-if="currentProduct.images.length>0" >
               <div class="carousel-item text-center" :class="index===0?'active':''" v-for="(img, index) in currentProduct.images" :key="index">
                 <img :src="img" class="img-thumbnail">
               </div>
             </div>
             <div class="carousel-inner" v-else >
               <div class="carousel-item active">
                 <img src="../../../assets/flash/1.png"  class="img-thumbnail" alt="">
               </div>
             </div>
       ...
       <div class="col-12 col-lg-4 stock-col" >
...
export default {
  components:{ImageView, ConfirmDialog, InfoDialog, Loading, AdminNavigation},
  data(){
    return{
  ...
  async created(){
    ...
    await db.collection('shopRegistration').doc(this.$route.params.shop_id).get().then(async(result)=> {
      ...
      await db.collection(this.owner.city).doc(this.owner.shopId).collection('products').doc(this.$route.params.product_id).onSnapshot(snapshot => {
        this.currentProduct = snapshot.data();
        console.log('current product', this.currentProduct)
        db.collection(this.owner.city).doc(this.owner.shopId).collection('products').doc(this.$route.params.product_id).collection('records').orderBy('time','desc').onSnapshot(snapshot => {
          this.stockHistories =[];
        ...
        db.collection(this.owner.city).doc(this.owner.shopId).collection('products').doc(this.$route.params.product_id).collection('relation').onSnapshot(snapshot=>{
          this.relations = [];
          snapshot.forEach(doc=>{
  ...
  methods:{
    ...
    async confirm(){
      ...
      const fileRef = storageRef.child(`/${this.owner.city}/${this.owner.shopId}/products/${this.$route.params.product_id}`);
    ...
    async deleteDoc(){
      await db.collection(this.owner.city).doc(this.owner.shopId).collection('products').doc(this.$route.params.product_id).collection('records').get().then(snapshot => {
        snapshot.forEach(doc => {
          doc.ref.delete();
        });
      })
      console.log('records deleted')
      await db.collection(this.owner.city).doc(this.owner.shopId).collection('products').doc(this.$route.params.product_id).collection('relation').get().then(snapshot=>{
        if(snapshot.size>0){
          snapshot.forEach(doc=>{
            doc.ref.delete();
          })
      ...
      await db.collection(this.owner.city).doc(this.owner.shopId).collection('products').doc(this.$route.params.product_id).delete();
      this.loadingActive = false;
      this.$router.back();
    ...
    async updateStock(){
      ...
      if(this.frontNumber!==''){
        ...
        // this.loadingActive = true;
        // this.loadingActive = true;
        await db.collection(this.owner.city).doc(this.owner.shopId).collection('products').doc(this.$route.params.product_id).update(
            {
              'stock':newStock.toString()
            }
        );
        const recordRef =await db.collection(this.owner.city).doc(this.owner.shopId).collection('products').doc(this.$route.params.product_id).collection('records');
        await recordRef.add(
            {
    ...
    async updateDiscount(){
      if(this.update_discount!==''){
        ...
        await db.collection(this.owner.city).doc(this.owner.shopId).collection('products').doc(this.$route.params.product_id).update(
          ...
          const recordRef = db.collection(this.owner.city).doc(this.owner.shopId).collection('products').doc(this.$route.params.product_id).collection('records');
          recordRef.add(
              {
                'id':this.currentProduct.id,
                'recordId':recordRef.id,
      ...
      if(this.update_waste!==''){
        ...
        await db.collection(this.owner.city).doc(this.owner.shopId).collection('products').doc(this.$route.params.product_id).update(
          ...
          if (this.allSrc.length > 0) {
            ...
            const imgRef = storageRef.child(`/${this.owner.city}/${this.owner.shopId}/products/${this.$route.params.product_id}/waste/${recordRef.id}`)
    ...
    async updateRelationTable(){
      if(this.relation.frontUnit!=='' && this.relation.backNumber!=='' && this.relation.sellPrice!==''){
        const relationRef = await  db.collection(this.owner.city).doc(this.owner.shopId).collection('products').doc(this.$route.params.product_id).collection('relation').doc();
    ...
    async deleteRelation(relation){
      await db.collection(this.owner.city).doc(this.owner.shopId).collection('products').doc(this.$route.params.product_id).collection('relation').doc(relation.id).delete().then(()=>{
        console.log('deleted')
      })
    },
