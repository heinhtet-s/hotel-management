  ...
  <ProductSelectModal :selectedProduct="selectedProduct" :owner="owner" :selectedRelatons="selectedRelations" v-if="showDialog" v-on:confirm-modal="confirmItem" v-on:close-modal="closeDialog"/>
    ...
    <div class="row" >
      ...
      <div class="d-flex p-1 align-items-center justify-center" style="z-index: 1000; background-color: var(--light);">
        ...
        <button class="btn mx-auto my-1 w-50 text-light text-uppercase fw-bold" style="background-color: var(--primary);"  :disabled="sellNowActive==false" @click="sellNow" v-print="'#printcontent'" data-bs-dismiss="modal" >{{$t('message.sell_now')}}</button>
      </div>
...
export default {
  ...
  async mounted() {
    ...
    this.selectedSellProduct.forEach(selection=>{
      this.totalPrice += parseInt(selection.sellPrice ) * parseInt(selection.count);
      this.costPerItem.push(parseInt(selection.sellPrice ) * parseInt(selection.count));
    })
  ...
  methods:{
    ...
    sellRecord(){
      console.log('sellRecord', this.checkoutItem)
      this.checkoutItem.countList.forEach((p, i)=>{
        let sellCount = parseInt(p)*parseInt(this.checkoutItem.backNumberList[i]);
        const productId = this.checkoutItem.productIdList[i];
        console.log('sell record',sellCount, productId)
        db.collection(this.shopInfo.city)
            .doc(this.shopInfo.id)
            .collection('products')
            .doc(productId).get().then(snapshot=>{
          let originalProduct = snapshot.data();
          console.log('org pro', originalProduct);
          let originalStock = originalProduct.stock;
          console.log('Original Stock', originalStock)
          let finalStock = 0;
          if(originalStock !==''){
            finalStock = parseInt(originalStock) - sellCount;
            db.collection(this.shopInfo.city)
                .doc(this.shopInfo.id)
                .collection('products')
                .doc(productId).update({'stock':finalStock+""});
          ...
          const recordRef = db.collection(this.shopInfo.city)
              .doc(this.shopInfo.id)
              .collection('products')
              .doc(this.checkoutItem.productIdList[i]).collection("records").doc();
          ...
          if(this.checkoutItem.countList.length === i+1){
            db.collection(this.owner.city).doc(this.owner.shopId).collection('tables').doc(this.selectedTable.id).collection('orders').get().then(snapshot=>{
              snapshot.docs.forEach((doc, ind)=> {
                console.log('order', ind,'size', snapshot.size)
                db.collection(this.owner.city).doc(this.owner.shopId).collection('tables').doc(this.selectedTable.id).collection('orders').doc(doc.data().id).delete();
                if(ind+1 === snapshot.size){
                  db.collection(this.owner.city).doc(this.owner.shopId).collection('tables').doc(this.selectedTable.id).update({
                    categoryType:"Free"
                  });
