<template>
  <admin-sidebar>
    ...
    <div class="">
      <div class="pb-1" style="
        ...
        <div class="d-lg-flex d-md-flex align-items-center rounded justify-content-between mb-2">
          <div class="d-flex align-items-center">
            <button @click="this.$router.back()" style="color: var(--text-color)" class="btn border-0 float-start">
              <i class="fas fa-chevron-left"> </i>
            </button>
            <router-link class="fs-5 p-1 mb-1 text-decoration-none" style="color: var(--text-color) !important"
              :class="$route.name === 'SellReport' ? 'route-active' : ''" :to="{ name: 'SellReport' }">
              {{ $t("message.invoices") }}</router-link>
            <span class="fs-5 mb-1 mx-1" style="color: var(--primary)">|</span>
            <router-link class="fs-5 mb-1 p-1 text-decoration-none" style="color: var(--text-color) !important"
              :class="$route.name === 'ProductSellReport' ? 'route-active' : ''" :to="{ name: 'ProductSellReport' }">
        ...
        <div class="">
          <div class="d-flex flex-lg-row flex-column justify-content-lg-between px-2">
            <div class="d-flex">
              <div class="d-flex p-1 mb-1" style="
                  background-color: var(--light);
                  border-radius: var(--border-radius);
                ">
                <span class="badge my-1 d-flex align-items-center me-1" style="
            ...
            <div class="date-panel">
              <div class="d-flex align-items-center rounded">
                ...
                <select class="form-select border-0 form-control bg-light mx-1"
                  :hidden="cashier.accountLevel !== 'admin'" @change="onChangeSellType($event)"
                  aria-label="Default select example">
                  <option value="Bar">Bar</option>
                  <option :value="kitchen.name" v-for="(kitchen, id) in kitchens" :selected="id === 0"
                    :key="kitchen.id">
                    {{ kitchen.name }}
                  </option>
                </select>
                <vue-excel-xlsx :data="filterExportData" :columns="columns" :file-name="fileName" :file-type="'xlsx'"
                  :sheet-name="fileName" class="btn btn-success ms-2 d-flex align-items-center border-success border-2">
      ...
      <div class="mt-3 mb-2 border-bottom border-1" style="height: 70vh; overflow-y: scroll">
            ...
            <tr v-for="(order, index) in filterInvoices" :key="index" class="align-middle">
              <td class="text-start">
                <span class="badge" style="
                  ...
                  " v-if="order.casherCode == 'C1'">Coffee Counter</span>
                <span class="badge text-light" style="width: 100px; background-color: var(--primary)"
                  v-if="order.casherCode == 'R1'">Restaurant</span>
                <span class="badge" style="width: 100px; background-color: var(--dark)"
                  v-if="order.casherCode == 'B1'">Bar</span>
              ...
              <td class="text-nowrap text-center">
                <router-link style="
                  ...
                  }" class="btn border-0 btn-sm me-1 text-light"><i class="fas fa-info-circle"></i>
                ...
                <router-link style="
                  ...
                  }" class="btn border-0 btn-sm text-white" v-if="cashier.editStatus === 'yes'">{{
  $t("message.refund")
}}
                </router-link>
              </td>
      ...
      <div v-if="invoices.length > perPage">
        ...
        <pagination :totalPages="pageCount" :perPage="perPage" :currentPage="currentPage" :pages="invoices"
...
export default {
  ...
  computed: {
    ...
    filterInvoices() {
      ...
      if (this.search !== "") {
        ...
        console.log(
          "fekofk",
          this.invoices.slice(this.offsetCount, this.offsetCount + this.perPage)
        );
  ...
  methods: {
    ...
    async onChangeSellType(e) {
      this.search = "";
      const sellType = e.target.value;
      if (sellType === "Restaurant") {
        this.selectedSellType = "R1";
      } else if (sellType === "Coffee Counter") {
        this.selectedSellType = "C1";
      } else {
        this.selectedSellType = "B1";
      }
      await this.getVoucher();
    },
    ...
    async getVoucher() {
      ...
      await db
        ...
        .where("casherCode", "==", this.selectedSellType)
