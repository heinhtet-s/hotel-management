  ...
  <admin-sidebar>
    ...
    <div class="row" style="overflow-y: hidden">
      <div class="d-flex align-items-center justify-content-between rounded mb-2">
        <div class="d-flex rounded align-items-center mt-2">
          <button @click="$router.back()" class="btn me-2 fs-4 float-start border-0" style="color: var(--primary)">
            ...
            <span> {{ $t("message.create_reservation") }} | {{ cashierAcc.counter }} </span>
          </button>
        </div>
      </div>
      <div class="col-12 col-lg-3">
        ...
        <form class="row align-items-center" style="margintop: 200px">
          <div class="col-12">
            <div class="form-floating mb-3">
              <input type="text" id="name" v-model="form.customerName" class="form-control" required
          ...
          <div class="col-12">
            <div class="form-floating mb-3">
              <input type="number" id="phone" v-model="form.customerPhone" class="form-control" required
          ...
          <div class="col-12">
            <div class="form-floating mb-3">
              <input type="number" id="people" v-model="form.peopleCount" class="form-control" required
          ...
          <div class="col-12">
            <div class="form-floating mb-3">
              <input type="number" id="table" v-model="form.tableCount" class="form-control" required
                placeholder="Enter Ms Code" />
          ...
          <div class="col-12">
            <div class="form-floating mb-3">
              <input type="date" id="date" v-model="form.startDate" class="form-control" @change="filterTableList"
                required />
          ...
          <div class="col-12">
            <div class="form-floating mb-3">
              <input type="time" id="time" v-model="form.startTime" class="form-control" required />
      ...
      <div class="col-12 col-lg-4 rounded">
        ...
        <div class="d-flex p-1" style="background-color: var(--background-color)">
          <button style="background-color: var(--primary); width: 100%" class="btn text-white" @click="submitSale">
            <!-- submit -->
            {{ $t("message.confirm") }}
          </button>
        </div>
...
export default {
  ...
  computed: {
    ...
    pageCountTable() {
      return Math.ceil(this.tableList / this.perPageTable);
    },
  ...
  methods: {
    ...
    async submitSale() {
      ...
      if (
        ...
        ReservationRef.set(data)
          .then(async () => {
            this.selectedSellProduct.forEach(async (doc) => {
              let orderRef = await db
                .collection(this.owner.city)
                .doc(this.owner.shopId)
                .collection("tableReservation")
            ...
            this.$store.commit("SET_TABLE_RESERVATION_DETAIL", {
              ...data,
        ...
        this.modalMessage = "Please fill the data completely!";
      }
    },
    ...
    checkProduct(payload) {
      ...
      if (
        ...
        //   .collection("orders")
        //   .doc();
        console.log("selected table", this.selectedProduct);
    ...
    increaseCount(order) {
      ...
      if (order.stock === "") {
        let sameIndex = this.selectedSellProduct.findIndex(
          (p) =>
            p.productId === order.productId && p.relation === order.relation
        );
        console.log("has");
        this.selectedSellProduct[sameIndex].count = parseInt(order.count) + 1;
      } else {
        console.log(order.relationNumber, "fjeiofff");
        ...
        if (
              ...
              if (totalCount <= relationCount) {
                ...
                let sameIndex = this.selectedSellProduct.findIndex(
                  (p) =>
                    ...
                    p.relation === order.relation
                );
                this.selectedSellProduct[sameIndex].count =
                  parseInt(order.count) + 1;
              } else {
                this.infoModalActive = true;
                this.modalMessage = "Out of stock";
              }
          ...
          let sameIndex = this.filterProductItem.product.findIndex(
            (p) => p.id === order.productId
          );
          if (this.filterProductItem.product[sameIndex].stock <= a) {
            a++;
            this.infoModalActive = true;
            this.modalMessage = "Out of stock!";
          } else {
    ...
    decreaseCount(order) {
      ...
      if (a > 1) {
        ...
        this.selectedSellProduct = this.selectedSellProduct.filter((el) => {
          return el.productId !== order.productId;
        });
      }
    },
